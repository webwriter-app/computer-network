...
export class IpAddress extends Address {
     ...
     * @returns 
     */
    override generateRandomAddress(database: Map<string, IpAddress>): IpAddress {
     ...
     * @returns 
     */
    override validateAddress(ip: string, database: Map<string, IpAddress>): boolean{
    ...
    generateNewIpForSubnet(database: Map<string, IpAddress>, oldIp: IpAddress, subnet: Subnet): IpAddress {
        //TODO: fix bug here: bug for subnet with cidr = 24
    
        if(IpAddress.matchesNetworkCidr(oldIp, subnet)){
            return oldIp;
        }
    ...
    static matchesNetworkCidr(ip: IpAddress, subnet: Subnet): boolean {
        ...
        let binaryNodeArray = ip.binaryOctets;
